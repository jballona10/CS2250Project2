...
int main()
{
	...
	// Function calls to insert data into array, read the array, and find the most frequent number(s)
	...
	do
	{
		...
		// stored here
		int randNum2[SIZE1];
		// pointers to accept largest number, second largest number, and their subscripts
		...
		two_largest(randNum2, SIZE1, &largest, &largestsub, &secondlargest, &secondlargestsub);
		...
		// do while loop to be able to initialize new random number generator
		do
		{
			cin >> doWhileCont;
		...
		while (doWhileCont != 'y' && doWhileCont != 'Y' && doWhileCont != 'n' && doWhileCont != 'N');
		cout << endl;
	}
	while (doWhileStop != -1);
...
/*
 * goes through the array of random numbers and finds the most frequent ones, then displays them.
 */
void frequent(int randnumarray[], int size)
{
	...
	// add 1 to the subscript corresponding to each number in the second array to count the frequency
	...
	// for loop finds the number of times that the most frequent number was generated in the first array
	for (int index = 0; index < 31; index++)
	{
		if (arrCounter[index] > maxvalue)
			maxvalue = arrCounter[index];
	}
...
/*
 * goes through the array of random numbers and finds the TWO most frequent ones and their subscripts
...
void two_largest(int randNumArr2[], int size, int* a, int* b, int* c, int* d)
{
	...
	// for loop to store the largest number in largest and second largest in secondlargest
	for (int i = 0; i < size; i++)
	{
		// if the number is bigger than the number stored in largest, store it in largest
		if (randNumArr2[i] > largest)
			largest = randNumArr2[i];
		// else if the number is less than OR equal to the largest, and its greater than the second largest,
